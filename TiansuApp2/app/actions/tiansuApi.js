 ...
 * ext：     扩展字段
 */
function fetchTiansuList(typeObj, opt, category, pageNo, ext) {
	return (dispatch) => {
	  dispatch({type: typeObj.START, opt: opt});
	  let reqUrl = `http://192.168.20.160:3001/alarmList/${category}/${PAGE_NUM}/${pageNo}`;
 ...
 * pageNo：当前加载的页码
 */
export function fetchAlarmList(opt, category, pageNo, ext) {
	RLOG('actions -> tiansuApi -> fetchAlarmList');
	return fetchTiansuList(FETCH_ALARM_DATA_STATUS, opt, category, pageNo, ext);
}
...
//  */
// export function fetchAlarmDay(day) {
// 	RLOG('actions -> gankApi -> fetchGankDay >>>>>>>>>>>>>> ');
// 	return (dispatch) => {
// 	  dispatch({type: FETCH_ALARM_DATA_STATUS.START, day});
// 	  let reqUrl = `http://gank.io/api/day/${day}`;
// 	  RLOG('fetchGankDayUrl：' + reqUrl);
// 	  return fetch(reqUrl)
// 				    .then((response) => response.json())
// 				    .then(
// 				    	(responseData) => dispatch({type: FETCH_ALARM_DATA_STATUS.SUCCESS, day, data: responseData}),
// 				    	(error) => dispatch({type: FETCH_ALARM_DATA_STATUS.FAILURE, day, error})
// 				    );
// 	};
// }