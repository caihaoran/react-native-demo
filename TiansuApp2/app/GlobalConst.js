export const DEBUG = false; // 调试模式
export const RDEBUG = false; // Redux调试模式

export const APP_MAIN_COLOR = '#30c9f0'; // APP主颜色
export const COMMON_SELECT_COLOR = 'rgba(255, 80, 0, 0.1)'; // 统一按下颜色
export const TITLE_BAR_HEIGHT = 55; // 自定义标题栏高度
export const COMMON_BACKGROUND_COLOR = '#F5FCFF'; // 统一背景颜色
export const DEBUG_BACKGROUND_COLOR = '#333'; // 统一背景颜色
export const DEBUG_BACKGROUND_COLOR2 = '#ABC'; // 统一背景颜色

export const APP_VERSION = 'v1.3'; // 版本信息
